    ...
    return (
        <div className="md:block col-span-3 ">
            <div className="grid px-4">
                ...
                {currentCategory && (
                    <div>
                        <p className="category-item-name" onClick={() => handleClickSelectedCategory(currentCategory)}>{currentCategory.name}</p>

                        {expandCategories && (
                            <div className="ml-4">
                                {expandCategories[currentCategory.name]?.map(item => (
                                    <p className="my-1" onClick={() => handleToggleExpand(item, currentCategory)}>{item.name}</p>
                                ))}
                            </div>
                        )}
            ...
            {expandCategories && (
                <div className="ml-4">
                    {expandCategories[sub.name] && expandCategories[sub.name]?.map(item => (
                        <p className="my-1 " onClick={() => handleToggleExpand(item, sub)}>{item.name}</p>
                    ))}
                </div>
            )}
