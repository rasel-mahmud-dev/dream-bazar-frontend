...
export type PaginationType = {
  where: PaginationWhereEnum, perPage: number, currentPage: number
};
...
const productReducer = (state: ProductStateType=initialState, action)=>{
  ...
  switch(action.type){
    ...
    case ACTION_TYPES.FETCH_PRODUCT : 
      ...
      let m = setLoadingStates(
      updatedState.loadingStates, "product_details", false)
    ...
    case ACTION_TYPES.CHANGE_PAGINATION :  
      const paginationIndex = updatedState.paginations
      .findIndex(pg=>pg.where === action.payload.where) 
      ...
      if(paginationIndex !== -1){
        oldPaginations[paginationIndex] = {
          ...
          currentPage: action.payload.currentPage 
            ...
            : oldPaginations[paginationIndex].currentPage + 1
        }
      } 
    ...
    default :
      ...
      return filterSidebar(state, action)
  }
}
