...
class AuthService {
    async login(req: Request, body: { email: string, password: string }) {

        const user = await AuthRepository.getLoggedUserInfo(body.email);

        if (!user) return throwError("You are not registered", 401)
        const isMatchPassword = await Bcrypt.comparePassword(body.password, user.password)
