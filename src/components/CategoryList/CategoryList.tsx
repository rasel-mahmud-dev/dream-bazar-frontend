...
function CategoryList(props) {
    ...
    function getCat(flatCategories, params) {
        ...
        if (params.pId && catTree) {
            ...
            // root level or base level all categories where parent id is = null
            let rootCategories = flatCategories.filter((a) => a.parentId === null);
            // store it state so that we will render in jsx
            // it first level category or root level
            // { 1 : [{…}, {…}, {…}, {…}, {…}, {…}, {…}] }
            updateSidebarCategory[levelNumber] = rootCategories;

            // now get pId root category that is given in url first params
            ...
            let lastLevelCategory = flatCategories.find((item) => item.name === catTree);
            ...
            lastLevelCategory.expand = true;
            lastLevelCategory.last = true;

            // find upper level category from last level category until found root category
            ...
            arrId.forEach((id, index) => {
                ...
                if (index + 1 === arrId.length) {
                    let lastItems = updateSidebarCategory[levelNumber];
                    updateSidebarCategory[levelNumber] = lastItems.map((itm) => {
                        return {
                            ...itm,
                            expand: itm._id === lastLevelCategory._id,
                        };
                    });
                }
            });
            ...
            // setLastParentSubCategories({
            //     lastParentId: lastLevelCategory._id,
            //     sub: flatCategories.filter(item => lastLevelCategory._id === item.parentId),
            //     levelNumber: levelNumber,
            // })

            setSidebarCategory(updateSidebarCategory);
            handleChangeCategory(lastLevelCategory, flatCategories)
        }
    ...
    function clickOnCategoryItem(item: CategoryType, levelNumber) {
        ...
        /* example
            ...
            then delete 4, 5, 6, 7 level category if they exist
        * */
    ...
    return (
        <div className="md:block col-span-3 ">
            <div className="grid px-4">
                    ...
                    (cat) =>
                        (!sidebarCategory[2] || cat.expand) && (
                            <div key={cat._id} className="ml-2 flex justify-between">
                                <li
                                    onClick={() => clickOnCategoryItem(cat, 1)}
                    ...
                    (cat) =>
                        (!sidebarCategory[3] || cat.expand) && (
                            <div key={cat._id} className="ml-2">
                                <li
                                    onClick={() => clickOnCategoryItem(cat, 2)}
                                    className={`category-item cursor-pointer ${cat.active || cat.expand ? "expanded-category" : "hidden"}
                                   ${cat.last ? "last-expand-category" : ""}
                            `}
                                >
                ...
                {sidebarCategory?.["3"]?.map(
                    (cat) =>
                        (!sidebarCategory[4] || cat.expand) && (
                            <div key={cat._id} className="ml-2">
                                <li
                                    onClick={() => clickOnCategoryItem(cat, 3)}
                ...
                {sidebarCategory?.["4"]?.map(
                    (cat) =>
                        (!sidebarCategory[5] || cat.expand) && (
                            <div key={cat._id} className="ml-2">
                                <li
                                    onClick={() => clickOnCategoryItem(cat, 4)}
                ...
                <div className="ml-4">
                    {lastParentSubCategories.sub &&
                        lastParentSubCategories.sub.map((item) => (
                            <div key={item._id}>
                                <h1
                                    onClick={() => handleExpandCategory(item, lastParentSubCategories.levelNumber)}
                                    className={`category-item cursor-pointer`}
                                >
                                    {item.name}
                                </h1>
