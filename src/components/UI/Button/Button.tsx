...
const Button: FC<BaseButtonProps> = (props) => {
    const { loading, theme, to, href, loaderClass, className, ...attributes } = props;
    ...
    if (to) {
        ...
        return (
            <button {...attributes} className={`btn rounded ${theme ? "btn-"+theme : ""} ${className} ${loading ? "btn-disable": "" }`}>
                {loading && <div className={`btn-loading-circle mr-1.5 dark:bg-red-500 ${loaderClass}`}>
                    <LoadingIcon />
                </div> }
                {props.children && <span>{props.children}</span>}
